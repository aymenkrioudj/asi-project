...
class _HomePageState extends State<HomePage> {
  List sessions = <Session>[
    Session("ASI Cours", "Fouad Dahak", "(AP2)", TimeOfDay(hour: 8, minute: 30),
        TimeOfDay(hour: 10, minute: 00)),
    Session("BDA Cours", "Karima Amrouche", "(A3)",
        TimeOfDay(hour: 10, minute: 10), TimeOfDay(hour: 11, minute: 30)),
    Session("ASI TD", "Sabrina Abdelaoui", "(CP7)",
        TimeOfDay(hour: 11, minute: 55), TimeOfDay(hour: 13, minute: 30)),
    Session("PGI TD", "Selma Khouri", "(CP9)", TimeOfDay(hour: 13, minute: 35),
...
//TODO : les methodes de scan QR code
  ...
  Future<void> scanQR() async {
    ...
    setState(() {
      ...
      if (_scanBarcode == "ASI Cours" &&
          compareDate(sessions[0].dateD, sessions[0].dateF)) {
        isSession = true;
      }
      if (_scanBarcode == "BDA Cours" &&
          compareDate(sessions[1].dateD, sessions[1].dateF)) {
        isSession = true;
      }
      if (_scanBarcode == "ASI TD" &&
          compareDate(sessions[2].dateD, sessions[2].dateF)) {
        isSession = true;
      }
      if (_scanBarcode == "PGI TD" &&
          compareDate(sessions[3].dateD, sessions[3].dateF)) {
        isSession = true;
      }
      if (_scanBarcode == "-1") {
        //cancel : do nothing
        ...
        CoolAlert.show(
          context: context,
          type: isSession ? CoolAlertType.success : CoolAlertType.error,
          title:
              isSession ? "Confirmed presence to :" : "Qr code not validated",
          text: isSession
              ? _scanBarcode + "\n" + hournow + "\n" + datenow
    ...
    return showDialog(
          ...
          builder: (context) => new AlertDialog(
            ...
            content: new Text('Do you want to exit an App',
                style: TextStyle(color: colorBlue1, fontFamily: "Titre1")),
            actions: <Widget>[
              new GestureDetector(
...
class Timeline extends StatelessWidget {
  ...
  Timeline(
      this.index, this.module, this.prof, this.salle, this.dateD, this.dateF);
...
class _ContentCardState extends State<ContentCard> {
  ...
  Widget build(BuildContext context) {
    return Card(
      color:
          compareDate(widget.dateD, widget.dateF) ? colorBlue3 : Colors.white,
      child: Container(
